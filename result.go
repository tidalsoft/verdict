...
// Class returns the check class that produced this result.
...
// constructor: Class S checks default to warn and reach block only for a
// specific field, only after a measured per-field promotion. This package
// has no visibility into fire counts or precision measurements -- that
// lifecycle happens outside this library -- and so cannot itself verify a
// promotion is warranted; what it can do is refuse the shortcut of reaching
// block by accident through the ordinary constructor. Use
// NewPromotedResult for a check whose promotion has already been
// authorized upstream.
func NewResult(checkID string, class Class, severity Severity, outcome Outcome) (Result, error) {
	...
	if class == ClassS && severity == SeverityBlock {
		return Result{}, fmt.Errorf("verdict: check %q: class S check cannot be constructed at block severity via NewResult; use NewPromotedResult once promotion has been authorized upstream", checkID)
	}
...
// platform-side decision (the promotion lifecycle itself happens outside
// this library). This function exists because this library is pure and
// cannot itself verify that a promotion was authorised -- it performs no
// I/O and sees only what the caller passes it -- so calling this
// constructor is itself the caller's assertion that promotion has already
// happened. Expected callers are resolved-ruleset evaluation code acting
// on a ruleset that already reflects the promotion, never check
// implementations deciding severity for themselves.
